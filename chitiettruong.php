...
$anhminhhoa = get_list("SELECT * FROM `duhocnghe` WHERE `id` = '" . $id . "'", $conn)[0]['anhminhhoa'];
$quocgia = get_list("SELECT * FROM `duhocnghe` WHERE `id` = '" . $id . "'", $conn)[0]['quocgia'];
...
.text p {
   ...
   <body>
      ...
      <style>
       ...
       <h2 style = "text-align: center; color: blue; "> <? echo get_list("SELECT * FROM `duhocnghe` WHERE `id` = '" . $id . "'", $conn)[0]['tieude']?></h2>
...
<div style="margin-top: 40px;"> <? echo get_list("SELECT * FROM `duhocnghe` WHERE `id` = '" . $id . "'", $conn)[0]['gioithieu']?></div>
<div class="content">
        <div>
        <img src="image/<?php echo $anhminhhoa?>" alt="Ảnh minh họa">
        <p style="textext-align:center"><?php echo get_list("SELECT * FROM `duhocnghe` WHERE `id` = '" . $id . "'", $conn)[0]['tenminhhoa']?></p>
        </div>   
          <span style="textext-align:center;"> Danh Sách Các Trường  </span>  </div>
           <?php 
for ($i = 1; $i <= 5; $i++) { 
    foreach (get_list("SELECT * FROM `duhocnghe` WHERE `id` = '" . $id . "'",$conn) as $row) { 
?>
    <div class="truong1">
...
<form id ="forma2" action="db/hsungtuyen.php" method="post">
    <input type="hidden" id="quocgia" name="quocgia" value="<?php echo $quocgia; ?>">
<input type="text" style="height: 40px;margin-top:30px; width:90%; margin-left:20px;" class="form-control show-select" id="chose-option-diemden-empty" placeholder="Họ Và Tên:" name="hoten">
<input type="text" style="height: 40px; margin-top:30px; width:90%; margin-left:20px;" class="form-control show-select" id="chose-option-diemden-empty" placeholder="Số Điện Thoại:"name="sdt">
   <input type="text" style="height: 40px; margin-top:30px; width:90%; margin-left:20px;" class="form-control show-select" id="chose-option-diemden-empty" placeholder="Email của bạn:"name="email">
  
    
             <select style="width: calc(100% - 20px); padding: 10px;margin-bottom: 20px;margin-top: 30px; width:90%;margin-left:20px; border: 1px solid #ccc; border-radius: 5px; font-size: 16px;" id="school" name="school" required>
                <option value="">Bạn muốn đăng ký du học tại trường nào?</option>
                <!--<option value="">Chọn Châu Lục</option>-->
          
           <?
           for ($i = 1; $i <= 5; $i++) {
    foreach (get_list("SELECT * FROM `duhocnghe` WHERE `id` = '" . $id . "'",$conn) as $row) {
        echo '<option value="' . $row['tenanh' . $i] . '">' . $row['tenanh' . $i]  . '</option>';
    }
}
           ?>
