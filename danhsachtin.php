...
<thead>
                                    <tr>
                                        <th class="whitespace-nowrap">Tên tin đăng</th>
                                        <th class="whitespace-nowrap">Ngày đăng</th>
                                        <th class="whitespace-nowrap">Thời hạn nộp</th>
                                        <th class="whitespace-nowrap">Lượt nộp </th>
                                        <th class="whitespace-nowrap">Lượt xem</th>
                                        <th class="whitespace-nowrap">Tình trạng tin</th>
...
<!--</div>-->
    ...
    <tbody>
         <?php foreach (get_list("SELECT * FROM `tin_tuyen_dung`",$conn) as $row) {?>
          <tr>
              <td><?= $row['chuc_danh'] ?>
              <div>Loại tin:  <?php echo $row['dich_vu_id'] == 1 ?"tin không phí" : "tin có phí" ?> | Mã tin: <?php echo $row['id']?>  </div>
...
<span class="magnifying-glass">&#128269;</span> Xem hồ sơ ứng viên phù hợp
              ...
              <td><?php echo strtotime($row['han_nop_ho_so']) > strtotime(date("Y-m-d")) ? date("d-m-Y",strtotime($row['han_nop_ho_so'])) :" đã hết hạn"?></td>
              <td><?php echo $row['luot_nop']?></td>
              <td><?php echo $row['luot_xem']?></td>
              <td><?php echo $row['tinh_trang_tin'] == 0 ? "đang ẩn" : "đang bật"   ?></td>
              <td>
    <div style="<?php echo $row['trang_thai'] == 1 ? 'background-color: #00FF00; border-radius: 10px; text-align: center;' : 'background-color: rgb(242, 148, 89); border-radius: 10px; text-align: center;'; ?>">
        <?php echo $row['trang_thai'] == 0 ? "Chờ duyệt" : "Đã duyệt"; ?>
    </div>
</td>

        
               <td>
    <button onclick="window.print()">Xuất Bản</button>
    <td>
    <div class="dropdown">
